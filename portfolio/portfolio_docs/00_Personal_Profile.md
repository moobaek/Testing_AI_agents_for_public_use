...
**2-stage inverter 제작** (전자회로(2), 김종선 교수님)
- **목표**: 높은 dB을 가진 2단 증폭기 설계
- **기술적 접근**:
  - PMOS와 NMOS의 beta값의 비를 이용하여 charge 부분과 discharge 부분의 크기 조정
  - Q=CV를 시분하여 i(t)=C*(dV/dt) 계산으로 뒷단 커페시터값 도출
  - MOS 공식을 사용하여 I(t)값 도출
  - 상승/하강시간 최적화로 증폭값 유지
- **역할**: 회로 설계, 보고서 작성

**링 발진기 제작 (MOS 사용)** (집적회로 설계, 김종선 교수님)
- **목표**: 1GHz 주파수 출력, 서로 다른 위상의 출력 두 가지 생성
- **3단 구성**:
  - **입력단**: NAND gate를 최소한의 크기로 구현, pulse 발생 시 발진기 작동
  - **발진단**: MOS를 사용한 인버터로 일정 주파수(1GHz) 출력, RC delay 계산 기반 설계
  - **출력단**: 서로 다른 위상의 출력 두 가지 생성
- **기술적 도전**:
  - 커페시터 설정 및 상승시간/하강시간 10% 제한
  - Cin와 Cout을 같게 함으로서 지터값 최소화
  - 마이크로 윈드 프로그램을 사용하여 반도체 형태로 회로 제작
  - 최소 면적과 전력 사용 목표
- **역할**: 회로 설계, 보고서 작성, 인터뷰 수행

**10Gbps CDR 제작** (4-2 IT종합설계, 졸업논문 및 프로젝트, 이종창 교수님)
- **목표**: 10Gbps 고주파 환경에서 작동하는 CDR 제작 (DLL 방식)
- **모듈 구성**:
  - **PD (파동 검출단)**: 외부 데이터와 내부 고정 주파수 비교하여 오차 출력
  - **증폭+잡음 제거**: 2-stage inverter 응용 증폭부, LPF를 통한 미세 잡음 제거
  - **VCO**: LPF 출력값에 따라 VCC-GND 조절하여 발진, 링발진기 응용
  - **마지막 모듈**: 마스터-슬레이브 방식으로 입력값 밀어주는 부분 설계
- **결과**: 산업용 MOS 사용, overshoot 제외하고 값 출력, PLL 방식으로 1Gbps 회로 추가 제작
- **역할**: 회로 제작, 발표

#### 광학 프로젝트

**10Gbps LD1, LD2 diode pspice 구현** (4-1 IT종합설계, 이종창 교수님)
- **목표**: 레이저 구성을 이해하고 플러스되는 부분과 차감되는 부분을 수학적으로 분리하여 논리회로적 구성
- **결과**: 매트랩 계산과 회로 부분 불일치 발생했으나, 10Gbps 값 출력되어 4-2 프로젝트 입력부분으로 활용
- **역할**: 증폭 부분 논리회로 구성, 발표

#### 마이크로파 프로젝트

**필터 및 안테나 제작** (마이크로파 공학(2), 이정해 교수님)
- **목표**: 정해진 주파수만 받도록 필터를 제작하여 외부에서 들어오는 값을 모듈하는 안테나 제작
- **기술적 접근**:
  - 전선의 굵기를 조절하여 저항 설정
  - 각도와 길이, 두께를 결정하여 동격 C와 L 설계
- **역할**: 개인 프로젝트, 설계와 보고서 작성

#### 컴퓨터 공학 프로젝트

**이미지 프로세싱 알고리즘에 FPGA-SoC 플랫폼 추가** (임베디드시스템종합설계, 구건재 교수님)
- **목표**: 기본적인 이미지 변환과 온오프에 RTL logic을 사용하여 다양한 목적 달성
- **기술 스택**: ARM 프로세서와 제작한 RTL logic 사용
- **결과**: FPGA 동작 불가로 인해 레나 이미지 변환 수준(회색, 선명하게, 흐리게, 모자이크 등)으로 수행
- **역할**: 코드 검색, 보고서 작성
...
**데이터 정합성 철학**:
- 정직하고 투명한 데이터 평가 추구
- 공정 데이터의 한계를 인정하고 현실적인 성과 제시
- 밸런스, test/train/val 완전 분리, 데이터 오염 방지 등 엄격한 검증 기준 적용
- AMS 시스템: 이상 탐지 93.7% 달성, 실질적 정확도는 데이터 한계를 고려하면 60~70%로 투명하게 공개

### 위기 극복 역량 (Problem Solving)

**"데이터가 없으면 직접 찾는다"**:
- 현장에서 가장 흔한 문제는 **"데이터가 없다"**는 상황
- 이 경우 고객사 DB 접속정보를 직접 받아 내부 구조를 직접 파악
- "어떻게든 찾아서 만든다"는 현장 중심적 문제 해결 매카니즘

### 협업 철학 (Collaboration Principles)

> [!NOTE] **고객 커뮤니케이션 3원칙**
> 1. **고객과 적대하지 않는다** - 신뢰 기반 협업
> 2. **정부 문서 외적으로 고객의 진짜 AI 목표를 따로 파악한다** - 화려한 보고서와 실제 니즈는 다르다
> 3. **기술 용어를 쓰지 않는다** - 현장 관리자가 이해할 수 있는 언어로 소통

**이중 커뮤니케이션 전략**:
- **고객에게**: 실용적이고 간결한 언어, 현장 중심적 설명
- **정부에게**: 화려하고 체계적인 문서, 기술적 성과 강조
...
**성과**:
- **GS 인증 1등급 취득 2개** (CoCTK, AMS - PDS 명칭으로 인증)
- **납품 실적**: 세아특수강, 포미아 (정식 납품), 일본 글로벌 기업
- **제조 풀필먼트 플랫폼**: DPS, AMS 시스템에서 사용자가 이해할 수 있는 제조 플랫폼 시연 다수 진행
- 데이터 전처리 및 분석 플랫폼 개발 (메인 PM, 기술 PM)
- 현 자동 전처리 플랫폼 개발 진행중 (백단 기술적 설계 완료)
- 사업 PM 수행중 (LLM을 활용하기 위한 지식 구조 플랫폼 개발)
- 문서 관리 및 개발 진행 관리에 PM 활동 수행 (Obsidian Design Origin 시스템 활용)
...
**리파코 컨설팅** (2023.04. ~ 2023.12., 충북과학기술원 주관 산업 디지털 전환 지원체계 구축사업, 리파코)
- 산업 디지털 전환 지원체계 구축사업 일환으로 컨설팅 진행 및 최종보고서 작성
- 컨설팅 최종보고서 완료 (2023.11.03 컨설팅 시작, 2023.12.26 최종보고서 완료)

**코아아이티 자연어 처리 & 챗봇 컨설팅** (2023.04. ~ 2023.12., 충북과학기술원 주관 산업 디지털 전환 지원체계 구축사업, 코아아이티)
- 자연어 처리 및 챗봇 개발 컨설팅
- 한약재/건강식품 데이터 분석 및 자연어 처리 (자연어처리_예제_231212_코아아이티.csv 기반)
- 효과 및 주의사항 정보 구조화 및 챗봇 응답 생성 시스템 설계
- CSV 기반 자연어 처리 파이프라인 구축 (한약재 이름-내용 구조 데이터 처리)
- BERT 기반 자연어 처리 모델 생성 및 학습 (자연어버트_모델_생성.ipynb: BertForSequenceClassification 사용)
- 공정 데이터 자연어 처리 (공정데이터.ipynb: 공정 기능 추출, 토큰화, 벡터화, DistilBERT 모델 적용)
- 자연어 BERT 모델 개발 및 적용 (BERT, DistilBERT 등 다양한 모델 실험)

**해태가루비 AI 솔루션 실증지원사업** (2023. ~ 2024.02., 해태가루비/더블유앤이케이)
- 2023년 AI 솔루션 실증지원사업 수행
- AI 솔루션 실증 및 지원
- 2024.02.02 완료보고서 작성
...
- 화려함보다는 **편리함** 우선
- 고성능 서버보다는 **초저성능에서도 돌아가는** 실용적 설계
- 복잡한 규칙이라도 **규칙 기반으로 정보화**해야 현장이 쓸 수 있음
- 최종 형태는 고전적이어도 **현장 친화적**이어야 함
...
**핵심 역량** (입사부터 지속):
1. **온톨로지 기반 구조화/정보화**: 입사(2020.09.01)부터 모든 프로젝트의 기반이 된 핵심 역량. ID 기반 온톨로지 맵, 문서 관계 추적, 구조화된 정보 관리로 53개+ 프로젝트와 298개+ 설계 문서를 체계적으로 관리. 피쉬본(FBS), 확률 네트워크, 패턴 정보화, LLM까지 모두 이 접근법의 연장선.
2. 오픈소스 도구를 원활하게 통합하고 최적화하는 기술 전문 지식
...
- **코딩 에이전트 (Original Development Plan)**: 외주 개발자 관리의 한계를 넘어서
  - 57개 프로젝트를 수행하면서 겪은 외주 개발자 산출물 관리 문제를 해결하기 위해 개발
...
1. **ID 기반 온톨로지 맵 문서 시스템의 의의** (입사부터 지속)
   - 입사(2020.09.01)부터 지속적으로 구축해온 온톨로지 기반 구조화/정보화 접근법의 진화
   - 5년간의 경험을 바탕으로 한 체계적인 정보 관리 시스템
...
- **Continuity & Safety 파이프라인**: AI 코드 위험 체크(8대 카테고리) 및 개발 컨텍스트 자동 복원 시스템
...
> [!IMPORTANT] **최종 목표: 휴먼 에이전트 시대 (Human Agent Era)**
> **"사람이 AI를 쓰는 게 아니라, AI가 필요할 때 사람을 쓰는다"**
> - 모든 개발이 서로 엮여있으며, **AI 편의를 가장 우선적으로 고려**
> - 사람은 AI가 필요할 때만 개입하는 **Human-in-the-Loop** 구조
> - 최종 목표는 완전한 사무/공장 자동화
