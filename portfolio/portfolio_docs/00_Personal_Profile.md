...
**핵심 역량** (입사부터 지속):
1. **온톨로지 기반 구조화/정보화**: 입사(2020.09)부터 모든 프로젝트의 기반이 된 핵심 역량. ID 기반 온톨로지 맵, 문서 관계 추적, 구조화된 정보 관리로 47개+ 프로젝트와 298개+ 설계 문서를 체계적으로 관리. 피쉬본(FBS), 확률 네트워크, 패턴 정보화, LLM까지 모두 이 접근법의 연장선.
