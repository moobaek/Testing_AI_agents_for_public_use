...
timeline
    ...
    2026 : Virtual Company Creation Agent v9.0 완성
         : AI-DB tri-mode 운영 모델 설계 (설계/개발/트러블슈팅)
         : pb.zst 이벤트 소싱 기반 데이터 정합성 관리
...
**v9.0 AI-DB 운영 모델 설계 성과 (2026)**: `pb.zst` 이벤트 스트림을 진실 소스로 하는 AI-DB 운영 구조를 설계하여, 설계/개발/트러블슈팅 tri-mode에서 동일한 데이터 계약으로 모델 상태를 관리하는 방식을 구현했습니다. Source 계층과 Projection 계층을 명확히 분리하여 데이터 정합성을 보장하는 구조를 확립했습니다.
...
mindmap
  root((ML Engineer<br/>5년 5개월 경력))
    ...
    AI 엔진 개발
      ...
      제어 최적화 AI 엔진
    v9.0 AI-DB 운영 (신규)
      pb.zst 이벤트 소싱 데이터 정합성
      AI-DB tri-mode 운영 (설계/개발/트러블슈팅)
...
| 출원일 | 특허명 | 상태 | 발명자 | ML 알고리즘 연계 |
| :--- | :--- | :--- | :--- | :--- |
| 2025.08.13 | 인공지능을 활용한 공정 최적화 관리를 위한 공정 관리 방법 및 그 시스템 | 출원완료<br/>심사청구완료 | **권순룡**, 최수영, 강용태, 안상윤 | 머신러닝 기반 특성요인도 자동 생성, 다단 클러스터링, 베이지안 네트워크 |
| 2025.03.28 | 공정 최적화를 위한 공정 관리 방법 및 그 시스템 | 등록결정<br/>(2025.12.09) | 강용태, **권순룡**, 김혜주 | 머신러닝 기반 특성요인도 자동 생성 |
| 2023.12.26 | 생산공정 에너지 및 설비상태 데이터 처리를 위한 전력 사용 패턴 분석 및 상태 분석 방법 | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | 머신러닝 학습, 설비 SoH 상태 분석 모델 |
| 2023.12.26 | 주기 데이터 분석 기반의 전력 추이 상황적 불량 이상 검증 방법 | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | 시계열 예측, 주파수 스펙트럼 분석 |
| 2021.12.07 | 설비 제어 특성 로그 데이터와 에너지 소비패턴 모델을 활용한 인공지능 기반의 이상 탐지 방법 및 장치 | 등록결정<br/>(2025.10.28) | 강용태, 김정호, **권순룡**, 김동기 | 머신러닝 학습, 상태 전이 모델, 에너지 소비 패턴 모델 |
