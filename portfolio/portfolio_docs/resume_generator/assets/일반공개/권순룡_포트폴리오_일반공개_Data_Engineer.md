...
## 🔬 특허 출원/등록 현황 (데이터 파이프라인 및 온톨로지 모델링 강조)

> [!NOTE] **데이터 엔지니어링의 기술적 기반**
> 아래 특허들은 데이터 파이프라인 구축, 온톨로지 모델링, 데이터 처리 방법론의 핵심 기술을 특허로 보호한 것입니다. 특히 **특허 1, 2**는 온톨로지 구조화/정보화의 통합 실현으로, 데이터 파이프라인 설계의 기술적 기반이 됩니다.

### 특허 출원/등록 현황 (5건)

| 출원일 | 특허명 | 출원번호 | 청구항 | 상태 | 발명자 | 데이터 엔지니어링 연계 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 2025.08.13 | **인공지능을 활용한 공정 최적화 관리를 위한 공정 관리 방법 및 그 시스템** | 10-2025-0040488<br/>(예정) | 10개 | 출원완료<br/>심사청구완료 | **권순룡**, 최수영, 강용태, 안상윤 | **온톨로지 구조화/정보화의 통합**: 특성요인도 자동 생성 → 관리 룰 생성 → 경로 탐색이 온톨로지 구조로 설계됨<br/>**데이터 파이프라인**: 공정 관련 데이터 수집 → 전처리 → 분석 → 결과 도출의 전체 파이프라인 특허화<br/>**DPS 5층 아키텍처**: 데이터 수집부터 분석까지의 전체 프로세스 기술적 기반 |
| 2025.03.28 | **공정 최적화를 위한 공정 관리 방법 및 그 시스템** | 10-2025-0040488 | 8개 | 등록결정<br/>(2025.12.09) | 강용태, **권순룡**, 김혜주 | **온톨로지 구조화**: 공정 관리 기술이 온톨로지 구조로 설계되어 데이터 모델링의 기초<br/>**등록결정 완료** |
| 2023.12.26 | **생산공정 에너지 및 설비상태 데이터 처리를 위한 전력 사용 패턴 분석 및 상태 분석 방법** | 1020230191965 | 6개 | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | **데이터 처리 방법론**: 에너지 미터기 및 센서 데이터 수집 → 전처리 → 머신러닝 학습 → 실시간 분석의 데이터 파이프라인 특허화<br/>**시계열 데이터 처리**: 설비 상태 및 SoH 분석을 위한 시계열 데이터 처리 기법 |
| 2023.12.26 | **주기 데이터 분석 기반의 전력 추이 상황적 불량 이상 검증 방법** | 1020230191969 | 5개 | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | **시계열 데이터 처리**: 시계열 분해 방식으로 주기 데이터 추출 → 주파수 스펙트럼 생성 → 시계열 예측의 데이터 처리 파이프라인 특허화<br/>**데이터 전처리**: 제어기 로그 데이터 + 전력 소비 데이터 결합 및 전처리 기법 |
| 2021.12.07 | **설비 제어 특성 로그 데이터와 에너지 소비패턴 모델을 활용한 인공지능 기반의 이상 탐지 방법 및 장치** | 1020210174274 | 10개 | 등록결정<br/>(2025.10.28) | 강용태, 김정호, **권순룡**, 김동기 | **데이터 통합**: 제어기 로그 데이터 + 전류 소비 데이터 결합 → 에너지 소비 패턴 데이터 생성의 데이터 통합 기법 특허화<br/>**데이터 전처리 방법론**: Regression Tree Series Denoising, Autoencoder Denoising, DNN/RNN/LSTM 기반 시계열 예측 노이즈 필터 등 다양한 데이터 전처리 기법<br/>**등록결정 완료** |

### 데이터 엔지니어링과 특허 기술의 연계

**특허 1 (2025.08.13) - 데이터 파이프라인 설계의 핵심 기술**:
- **데이터 수집**: 공정 관련 데이터 수집 방법론 특허화
- **데이터 전처리**: Regression Tree Series Denoising, Autoencoder Denoising, DNN/RNN/LSTM 기반 시계열 예측 노이즈 필터, STL 분해, DWT 유사도 기법 등 다양한 데이터 전처리 기법
- **온톨로지 구조화**: 데이터를 온톨로지 구조로 설계하여 DPS 5층 아키텍처의 기술적 기반
- **데이터 분석**: 계층 클러스터링, 중요도 랭킹, 다단 클러스터링 등 데이터 분석 기법

**특허 3, 4, 5 (에너지 패턴 분석) - 시계열 데이터 처리의 기술적 기반**:
- **시계열 데이터 처리**: 주기 데이터 추출, 주파수 스펙트럼 분석, 시계열 예측을 통합한 데이터 처리 파이프라인
- **데이터 통합**: 제어기 로그 데이터 + 전류 소비 데이터 결합 방법론
- **데이터 전처리**: 다양한 노이즈 제거 및 데이터 정규화 기법
