...
timeline
    ...
    2022 : CoCTK 엔진 총괄 설계 및 개발
         : 롯데알루미늄 QMS 백엔드 아키텍처 설계
    2023 : 공정 불량 예측 AI 엔진 개발
         : 생산공정 에너지 데이터 패턴 분석 아키텍처 설계
         : NLP 및 챗봇 컨설팅 프로젝트
    2024 : AMS (AI 종합 플랫폼) 아키텍처 설계
         : CoCTK GS 인증 1등급 취득
         : DPS 5층 아키텍처 설계 및 개발
         : 세아특수강 포미아 DX 실증센터 아키텍처 설계
...
```mermaid
mindmap
  root((Solutions Architect<br/>5년 5개월 경력))
    시스템 아키텍처 설계
      5층 아키텍처 설계 (DPS)
      Microservices 아키텍처
      Multi-Agent 시스템 설계
...
```mermaid
graph TB
    subgraph "핵심 아키텍처 프로젝트 (Job Relevance 높은 순)"
        DPS[DPS<br/>relevance_score: 98<br/>5층 아키텍처<br/>Microservices<br/>Neo4j GraphDB<br/>서버-엣지 하이브리드]
        FMEA[FMEA 자동화<br/>relevance_score: 95<br/>Multi-Agent 시스템<br/>Master Orchestrator<br/>8개 Sub-Agent<br/>Phase 0~5 워크플로우]
        VCC[Virtual Company Creation<br/>relevance_score: 92<br/>Dual-Tier AI<br/>87% 비용 절감<br/>GFS 특화 DB<br/>Decoupled Intelligence]
        AMS[AMS<br/>relevance_score: 90<br/>8단계 파이프라인<br/>4M2E 온톨로지<br/>지식 그래프 플랫폼]
        ORIGIN[Original Development Plan<br/>relevance_score: 88<br/>LangGraph/CrewAI<br/>Phase 0-13 워크플로우<br/>28개 Few-shot Rules]
        FACTORY[Factory Ontology Manager<br/>relevance_score: 85<br/>4M2E 관계 정의<br/>OEM/ODM 자동화<br/>다이어그램 기반]
        PLATFORM[Platform All<br/>relevance_score: 82<br/>통합 플랫폼 생태계<br/>API 인터페이스<br/>데이터 흐름 설계]
    end
    
    subgraph "기반 기술"
        CoCTK[CoCTK<br/>relevance_score: 70<br/>엔진 총괄 설계<br/>모듈화 체계화]
        FBS[FBS 엔진<br/>relevance_score: 65<br/>확률 네트워크 기초]
    end
    
    %% 아키텍처 관계
    DPS -->|"5층 아키텍처 경험"| VCC
    DPS -->|"Neo4j 경험"| AMS
    ORIGIN -->|"워크플로우 오케스트레이션"| FMEA
    ORIGIN -->|"ID 기반 온톨로지"| FACTORY
    VCC -->|"Dual-Tier AI 경험"| AMS
    AMS -->|"플랫폼 통합"| PLATFORM
    DPS -->|"플랫폼 통합"| PLATFORM
    CoCTK -->|"플랫폼 통합"| PLATFORM
    FBS -->|"확률 네트워크 기초"| DPS
    
    %% 데이터 아키텍처 관계
    DPS -->|"확률 네트워크 저장"| AMS
    AMS -->|"4M2E 온톨로지"| FACTORY
    
    style DPS fill:#e74c3c,color:#fff,stroke-width:3px
    style FMEA fill:#3498db,color:#fff,stroke-width:2px
    style VCC fill:#9b59b6,color:#fff,stroke-width:2px
    style AMS fill:#1abc9c,color:#fff
    style ORIGIN fill:#f39c12,color:#fff
...
```mermaid
graph TB
    subgraph "Solutions Architect 성과 (2020.09~2026.01)"
        subgraph "시스템 아키텍처"
            A1[5층 아키텍처<br/>Microservices<br/>Neo4j GraphDB<br/>서버-엣지 하이브리드]
            A2[Multi-Agent 시스템<br/>Master Orchestrator<br/>8개 Sub-Agent<br/>Phase 0~5 워크플로우]
            A3[Dual-Tier AI<br/>87% 비용 절감<br/>GFS 특화 DB<br/>Decoupled Intelligence]
        end
        
        subgraph "데이터 아키텍처"
            B1[Neo4j GraphDB<br/>4M2E 온톨로지<br/>지식 그래프 플랫폼<br/>확률 네트워크 저장]
            B2[GFS 특화 DB<br/>PostgreSQL-Inspired<br/>엔터프라이즈급<br/>데이터 무결성]
        end
        ...
        subgraph "통합 플랫폼"
            D1[Platform All<br/>AMS, DPS, CoCTK<br/>통합 생태계<br/>API 인터페이스]
        end
    ...
    style B2 fill:#1abc9c,color:#fff
...
| :--- | :--- | :--- | :--- | :--- |
| 2025.08.13 | 인공지능을 활용한 공정 최적화 관리를 위한 공정 관리 방법 및 그 시스템 | 출원완료<br/>심사청구완료 | **권순룡**, 최수영, 강용태, 안상윤 | 온톨로지 구조화/정보화 통합, 시스템 통합, DPS 5층 아키텍처 기반 |
| 2025.03.28 | 공정 최적화를 위한 공정 관리 방법 및 그 시스템 | 등록결정<br/>(2025.12.09) | 강용태, **권순룡**, 김혜주 | 온톨로지 구조화, 시스템 아키텍처 기초 |
| 2023.12.26 | 생산공정 에너지 및 설비상태 데이터 처리를 위한 전력 사용 패턴 분석 및 상태 분석 방법 | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | 시스템 아키텍처, 데이터 파이프라인 설계 |
| 2023.12.26 | 주기 데이터 분석 기반의 전력 추이 상황적 불량 이상 검증 방법 | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | 시계열 데이터 처리 시스템 |
| 2021.12.07 | 설비 제어 특성 로그 데이터와 에너지 소비패턴 모델을 활용한 인공지능 기반의 이상 탐지 방법 및 장치 | 등록결정<br/>(2025.10.28) | 강용태, 김정호, **권순룡**, 김동기 | 데이터 통합 시스템 |
