...
```mermaid
graph TB
    subgraph "권순룡 포트폴리오 (53개+ 프로젝트)"
        Root[[권순룡 포트폴리오]]
        
        subgraph "온톨로지 & 지식그래프 (Job Relevance 높은 순)"
            ONTOFLOW[OntoFlow<br/>relevance_score: 98<br/>온톨로지 기반 지식그래프 플랫폼<br/>엔티티/관계 자동 추출<br/>298개+ 문서 관리]
            DPS[DPS<br/>relevance_score: 95<br/>5층 아키텍처<br/>Neo4j 지식그래프<br/>데이터 메쉬 아키텍처]
            FACTORY[Factory Ontology Manager<br/>relevance_score: 92<br/>제조 공정 온톨로지 관리<br/>자연어 파싱<br/>DB Grounding]
            AMS[AMS<br/>relevance_score: 88<br/>베이지안 네트워크<br/>이상탐지 93.7%<br/>온톨로지 통합]
        end
        
        subgraph "AI Agent & 파이프라인"
            PMAGENT[PM Agent<br/>relevance_score: 85<br/>32개 MCP 서버<br/>LangGraph 자연어 인터페이스<br/>데이터 파이프라인]
            FMEA[FMEA 자동화<br/>relevance_score: 80<br/>Multi-Agent 8개 협업<br/>LangGraph 기반]
    ...
    style ONTOFLOW fill:#e74c3c,color:#fff,stroke-width:3px
    style DPS fill:#3498db,color:#fff,stroke-width:2px
    style FACTORY fill:#9b59b6,color:#fff,stroke-width:2px
    style AMS fill:#e67e22,color:#fff
    style PMAGENT fill:#1abc9c,color:#fff
...
```mermaid
graph TB
    subgraph "온톨로지전문가 Data Engineer 성과 (2020.09~2026.01)"
        subgraph "온톨로지 & 지식그래프 성과"
            A1[OntoFlow<br/>298개+ 문서<br/>온톨로지 자동 추출]
            A2[DPS<br/>Neo4j 지식그래프<br/>4M2E 관계]
            A3[Factory Ontology<br/>제조 공정 온톨로지<br/>자연어 파싱]
            A4[AMS<br/>이상탐지 93.7%<br/>베이지안 네트워크]
        end
        
        subgraph "데이터 파이프라인 성과"
            B1[DPS<br/>5층 아키텍처<br/>데이터 메쉬]
            B2[Kafka<br/>실시간 파이프라인<br/>스트리밍 처리]
            B3[PM Agent<br/>32개 MCP 서버<br/>파이프라인 오케스트레이션]
        end
        
        subgraph "AI 서비스 성과"
            C1[LangGraph<br/>Multi-Agent<br/>AI 서비스]
            C2[FMEA<br/>8개 Sub-Agent<br/>협업 시스템]
            C3[PM Agent<br/>LangGraph<br/>자연어 인터페이스]
        end
        
        subgraph "인증 및 특허"
            D1[GS 인증<br/>1등급 2개<br/>CoCTK, AMS-PDS]
            D2[특허<br/>출원/등록 5건<br/>온톨로지 관련]
            D3[논문<br/>발표 10편<br/>온톨로지/지식그래프]
        end
    end
...
```mermaid
timeline
    title 5년 5개월간의 온톨로지 & 지식 그래프 여정
    2020 : FBS 엔진 초기 개발 (온톨로지 구조화 시작)
         : 에스에이치 품질 예측 프로젝트
    2021 : 가상 센서 및 제어 최적화 프로젝트
         : 전력 데이터 예측 AI 엔진 개발
    2022 : CoCTK 엔진 총괄 설계 (온톨로지 구조화 실현)
         : 롯데알루미늄 QMS 백엔드 개발
    2023 : 생산공정 에너지 데이터 패턴 분석 (패턴 정보화)
         : 공정 불량 예측 AI 엔진 개발
    2024 : DPS 5층 아키텍처 지식기반 플랫폼 (Neo4j 지식그래프)
         : AMS 베이지안 네트워크 기반 이상탐지 (온톨로지 통합)
    2025 : OntoFlow 온톨로지 기반 지식그래프 플랫폼 (엔티티/관계 자동 추출)
         : Factory Ontology Manager (제조 공정 온톨로지 관리)
         : FMEA 자동화 Multi-Agent 시스템 (LangGraph)
...
graph TB
    ...
    subgraph "기반 기술"
        CoCTK[CoCTK<br/>relevance_score: 75<br/>데이터 전처리<br/>상관관계 분석]
        ORIGIN[Original Development Plan<br/>relevance_score: 70<br/>ID 기반 온톨로지 맵<br/>298개+ 설계 문서]
    end
    
    %% 연결 관계
    ORIGIN -->|"ID 기반 온톨로지 맵 확장"| ONTOFLOW
    DPS -->|"Neo4j 지식그래프 경험 활용"| ONTOFLOW
    DPS -->|"5층 아키텍처 기반"| AMS
    AMS -->|"베이지안 네트워크 기반 추론"| FMEA
    ONTOFLOW -->|"온톨로지 모델링 경험 확장"| FACTORY
    PMAGENT -->|"MCP 파이프라인 연동"| ONTOFLOW
...
| :--- | :--- | :--- | :--- | :--- |
| 2025.08.13 | **인공지능을 활용한 공정 최적화 관리를 위한 공정 관리 방법 및 그 시스템** | 출원완료<br/>심사청구완료 | **권순룡**, 최수영, 강용태, 안상윤 | **온톨로지 구조화/정보화 통합**: 특성요인도 자동 생성(피쉬본 = 구조화된 인과관계) → 관리 룰 생성(구조화된 규칙) → 경로 탐색(확률 네트워크 = 구조화된 확률 관계)이 온톨로지 구조로 설계됨. **AMS 프로젝트**의 핵심 기술 특허화 (FBS Module, RMS Module, 확률 네트워크 기반 원인 분석) |
| 2025.03.28 | **공정 최적화를 위한 공정 관리 방법 및 그 시스템** | 등록결정<br/>(2025.12.09) | 강용태, **권순룡**, 김혜주 | **온톨로지 구조화의 확장**: 공정 관리 기술이 온톨로지 구조로 설계되어 AMS에 통합. 피쉬본 기반 공정 관리 = 온톨로지 구조화의 실무 적용. **AMS 프로젝트**의 공정 관리 기술 특허화 |
| 2023.12.26 | **생산공정 에너지 및 설비상태 데이터 처리를 위한 전력 사용 패턴 분석 및 상태 분석 방법** | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | **패턴 정보화의 특허화**: 에너지 패턴 분석 기술이 구조화된 온톨로지 형태로 설계됨. 패턴 정보화 = 온톨로지 정보화의 핵심 실현. **에너지 패턴 분석 프로젝트**의 핵심 기술 특허화 |
| 2023.12.26 | **주기 데이터 분석 기반의 전력 추이 상황적 불량 이상 검증 방법** | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | **패턴 정보화의 특허화**: 에너지 패턴 분석 기술이 구조화된 온톨로지 형태로 설계됨. 시계열 분해 방식으로 주기 데이터 추출 → 주파수 스펙트럼 기반 전력 소비 패턴 데이터 생성 → 시계열 예측 기반 불량/이상 감지. **에너지 패턴 분석 프로젝트**의 주기 분석 기법 특허화 |
| 2021.12.07 | **설비 제어 특성 로그 데이터와 에너지 소비패턴 모델을 활용한 인공지능 기반의 이상 탐지 방법 및 장치** | 등록결정<br/>(2025.10.28) | 강용태, 김정호, **권순룡**, 김동기 | **온톨로지 정보화의 기초**: 에너지 소비 패턴 모델이 온톨로지 구조의 기초가 됨. 패턴 정보화 접근법의 초기 특허. 제어기 로그 데이터 + 전류 소비 데이터 결합 → 에너지 소비 패턴 데이터 생성 → 머신러닝 기반 이상 탐지 모델. **에너지 패턴 분석 기술의 초기 특허**. AMS Pattern Module의 기초 기술 |
