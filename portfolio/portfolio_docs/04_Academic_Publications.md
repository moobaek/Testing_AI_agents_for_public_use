...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 2025.08.13 | **인공지능을 활용한 공정 최적화 관리를 위한 공정 관리 방법 및 그 시스템** | 10-2025-0040488<br/>(예정) | - | 10개 | 출원완료<br/>심사청구완료 | **권순룡**, 최수영, 강용태, 안상윤 | **온톨로지 구조화/정보화의 통합**: 특성요인도 자동 생성(피쉬본 = 구조화된 인과관계) → 관리 룰 생성(구조화된 규칙) → 경로 탐색(확률 네트워크 = 구조화된 확률 관계)이 온톨로지 구조로 설계됨. 온톨로지 구조화/정보화 접근법의 통합 실현. 머신러닝 기반 특성요인도(Fishbone) 자동 생성 → 관리 룰 자동 생성 → 노드 이동 추론을 통한 경로 탐색으로 이상 원인 파악 및 해결 방안 제시. **AMS 프로젝트**의 핵심 기술 특허화 (FBS Module, RMS Module, 확률 네트워크 기반 원인 분석). ID 기반으로 FBS Module(`03_FBS/fish_born_making.py`), RMS Module(`04_RMS/cluster_auto_Binarization.py`), 베이지안 네트워크(`05_AMS_dev/bayesian_network_analyzer.py`) 간 관계 추적. **국가연구개발사업 지원** (에너지수요관리핵심기술개발) |
| 2025.03.28 | **공정 최적화를 위한 공정 관리 방법 및 그 시스템** | 10-2025-0040488 | - | 8개 | 등록결정<br/>(2025.12.09)<br/>설정등록 마감<br/>2026.03.09 | 강용태, **권순룡**, 김혜주 | **온톨로지 구조화의 확장**: 공정 관리 기술이 온톨로지 구조로 설계되어 AMS에 통합. 피쉬본 기반 공정 관리 = 온톨로지 구조화의 실무 적용. 머신러닝 기반 특성요인도 자동 생성 및 관리 룰 생성 → 이상 상황 감지 및 원인 분석. **AMS 프로젝트**의 공정 관리 기술 특허화. ID 시스템으로 공정-관리 룰-이상 상황 관계 추적. **등록결정 완료**. **참고문헌**: KR1020220090681, KR1020220120456, KR1020230085776 (특허 5와 연계) |
| 2023.12.26 | **생산공정 에너지 및 설비상태 데이터 처리를 위한 전력 사용 패턴 분석 및 상태 분석 방법** | 1020230191965 | 1020250100428<br/>(2025.07.03) | 6개 | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | **패턴 정보화의 특허화**: 에너지 패턴 분석 기술이 구조화된 온톨로지 형태로 설계됨. 패턴 정보화 = 온톨로지 정보화의 핵심 실현. 머신러닝 기반 설비 SoH(State of Health) 상태 분석 모델 생성 및 실시간 판단. **에너지 패턴 분석 프로젝트**의 핵심 기술 특허화. 특허 5와 온톨로지 관계로 연결되어 AMS Pattern Module에 통합. 논문(2023.07)과 연계. **국가연구개발사업 지원** (에너지수요관리핵심기술개발사업) |
| 2023.12.26 | **주기 데이터 분석 기반의 전력 추이 상황적 불량 이상 검증 방법** | 1020230191969 | 1020250100432<br/>(2025.07.03) | 5개 | 공개<br/>심사청구완료 | 최수영, 강용태, **권순룡**, 이상훈 | **패턴 정보화의 특허화**: 에너지 패턴 분석 기술이 구조화된 온톨로지 형태로 설계됨. 패턴 정보화 = 온톨로지 정보화의 핵심 실현. 시계열 분해 방식으로 주기 데이터 추출 → 주파수 스펙트럼 기반 전력 소비 패턴 데이터 생성 → 시계열 예측 기반 불량/이상 감지. **에너지 패턴 분석 프로젝트**의 주기 분석 기법 특허화. 특허 5와 온톨로지 관계로 연결되어 AMS Pattern Module에 통합. **국가연구개발사업 지원** (에너지수요관리핵심기술개발사업) |
...
**특허 1: 인공지능을 활용한 공정 최적화 관리를 위한 공정 관리 방법 및 그 시스템** (2025.08.13)
- **청구항 수**: 10개
- **핵심 기술**: 공정 관련 데이터 수집 → 머신러닝 기반 클러스터링으로 특성요인도(Fishbone) 자동 생성 → 다단 클러스터링으로 관리 룰 자동 생성 → 노드 이동 추론(에너지 효율/확률 관계)을 통한 경로 탐색으로 이상 원인 파악 및 해결 방안 제시
- **IPC/CPC**: G05B 23/02, G05B 19/418
- **프로젝트 연계**: AMS 프로젝트 (`project.ams`) - FBS Module, RMS Module, 베이지안 네트워크 기반 원인 분석 기술의 통합 특허화
- **국가연구개발사업**: 에너지수요관리핵심기술개발(에특), 과제번호 20222020900031, 연구기간 2022.04.01 ~ 2024.12.31
- **기술적 특징 상세**: 
  - **특성요인도 자동 생성**: 
    - 데이터 전처리: Regression Tree Series Denoising, Autoencoder Denoising, DNN/RNN/LSTM 기반 시계열 예측 노이즈 필터, STL 분해, DWT 유사도 기법 등
    - 주기 패턴 분석: FFT(Fast Fourier Transform)를 통한 시간 도메인 → 주파수 도메인 변환으로 센서 정보 파악
    - 계층 클러스터링: 데이터를 계층적으로 군집하여 특성요인도의 구조 생성
    - 중요도 랭킹: RandomForest 중요도, Linear Regressor 계수, 상관계수 종합 산출로 특정 결과에 가장 영향을 미친 센서 식별
  - **관리 룰 자동 생성**: 
    - 다단 클러스터링: 1단계 DBSCAN 밀도기반 초기 군집화 → 2단계 K-means 세분화 → 최적화 및 조정 단계
    - 이진화: 유전 알고리즘 기반으로 예측 정확도가 높게 0~N 레벨 설정 → 안전/위험 구간 이진화
    - 안전/위험 구간 산정: RandomForest 중요도, Linear Regressor 계수, Naive Bayes 변수 기여도, 상관계수 비중 반영
  - **경로 탐색**: 
    - 에너지 효율 기반: 각 센서 데이터의 에너지 효율 계산 → 선순위 확인 순서 생성 → 노드 이동 추론
    - 확률 네트워크 기반: 이상 상황 발생 전후의 사전 확률과 사후 확률 계산 → 확률 관계 그래프 생성 → 토큰화하여 LLM에 입력 가능
  - **가상 센서 데이터 생성**: 특정 센서 값 누락 시 보유 중인 다른 센서 데이터를 활용한 예측 모델 학습
- **법적 상태**: 출원완료, 심사청구완료

**특허 2: 공정 최적화를 위한 공정 관리 방법 및 그 시스템** (2025.03.28)
- **청구항 수**: 8개
- **핵심 기술**: 공정 관련 데이터 수집 → 머신러닝 기반 특성요인도 자동 생성 → 관리 룰 생성 → 이상 상황 감지 및 원인 분석
- **IPC/CPC**: G05B 23/02, G05B 19/418
- **프로젝트 연계**: AMS 프로젝트 (`project.ams`) - 공정 관리 기술 특허화
- **참고문헌**: KR1020220090681, KR1020220120456, KR1020230085776 (특허 5와 연계)
- **법적 상태**: 등록결정(2025.12.09), 설정등록 마감일 2026.03.09 - **등록결정 완료**

**특허 3: 전력 사용 패턴 분석 및 상태 분석 방법** (2023.12.26)
- **청구항 수**: 6개
...
**특허 4: 주기 데이터 분석 기반 전력 추이 상황적 불량 이상 검증 방법** (2023.12.26)
- **청구항 수**: 5개
...
**특허 5: 설비 제어 특성 로그 데이터와 에너지 소비패턴 모델을 활용한 인공지능 기반 이상 탐지** (2021.12.07)
...
1. **에너지 기반 설비 진단의 체계화**: 전력/에너지 데이터만으로 설비 상태를 진단하는 방법론을 특허로 보호하여, 센서 설치 없이도 설비 모니터링이 가능한 기술 경쟁력을 확보했습니다. (특허 3, 4, 5)

2. **시계열 분석 기법의 고도화**: 주기 데이터 추출, 주파수 스펙트럼 분석, 시계열 예측을 통합한 이상 검증 방법을 특허화하여, AMS Pattern Module의 핵심 알고리즘으로 발전시켰습니다. (특허 4)

3. **실시간 이상 탐지 시스템**: 제어 로그와 에너지 데이터를 결합한 실시간 이상 탐지 방법을 특허화하여, 제조 현장의 즉각적인 대응이 가능한 시스템을 구축했습니다. (특허 5)

4. **AI 기반 공정 관리 자동화**: 머신러닝을 활용한 특성요인도 자동 생성 및 관리 룰 자동 생성 기술을 특허화하여, 인력이나 기술이 부족한 중소규모 공장에서도 공정 관리가 가능한 시스템을 구축했습니다. (특허 1, 2)

5. **확률 네트워크 기반 원인 분석**: 노드 이동 추론을 통한 경로 탐색 기술을 특허화하여, 이상 상황 발생 시 최적의 원인 경로를 파악하고 해결 방안을 제시하는 기술을 확보했습니다. (특허 1)

6. **데이터 전처리 방법론의 체계화**: 다양한 노이즈 제거 및 데이터 정규화 기법을 특허에 명시하여, 제조 현장의 다양한 데이터 품질 문제에 대응할 수 있는 기술을 확보했습니다. (특허 1, 5)
...
| :--- | :--- | :--- |
| 에너지 소비 패턴 모델 (특허 5) | 상태 전이 모델 기반 이상 탐지 | 2021년 초기 특허 → 2023년 SoH 분석 특허로 발전 |
| 전력 사용 패턴 분석 (특허 3) | 설비 SoH 상태 분석 | 에너지 패턴 분석 프로젝트 → AMS Pattern Module 통합 |
| 주기 데이터 분석 (특허 4) | 주파수 도메인 분석 | 시계열 분해 → 주파수 스펙트럼 → AMS 패턴 분석 알고리즘 |
...
**특허 포트폴리오 구성**:
- **총 특허 출원**: 5건 (2건 등록결정 완료, 3건 심사 진행 중)
- **총 청구항 수**: 39개 (특허 1: 10개, 특허 2: 8개, 특허 3: 6개, 특허 4: 5개, 특허 5: 10개)
- **기술 분야별 분류**:
  - 에너지 패턴 분석 분야: 3건 (특허 3, 4, 5) - 1건 등록결정 완료
  - AMS 공정 관리 분야: 2건 (특허 1, 2) - 1건 등록결정 완료

**기술 경쟁력**:
- **에너지 기반 설비 진단 분야**: 3건의 특허로 기술 우위 확보
  - 특허 5 (2021): 제어 로그 + 에너지 소비 패턴 결합 (등록결정 완료)
  - 특허 3 (2023): 전력 사용 패턴 분석 및 SoH 분석
  - 특허 4 (2023): 주기 데이터 분석 기반 이상 검증
- **AI 기반 공정 관리 분야**: 2건의 특허로 기술 우위 확보
  - 특허 2 (2025): 공정 최적화를 위한 공정 관리 방법 (등록결정 완료)
  - 특허 1 (2025): AI 공정 최적화 관리 방법 및 시스템 (심사 진행 중)
...
1. **기술적 신뢰도 확보**: 단순히 솔루션을 구축하는 것에 그치지 않고, 그 기저의 알고리즘과 방법론을 학술적으로 검증받고 특허로 보호했습니다. 총 **5건의 특허 출원** (2건 등록결정 완료, 3건 심사 진행 중, 총 청구항 39개)을 통해 지식 재산권을 확보했습니다. 특히 **국가연구개발사업 지원**을 받은 특허들이 있어 정부의 기술 개발 방향과 일치하며 산업계 적용 가능성을 입증받았습니다.
