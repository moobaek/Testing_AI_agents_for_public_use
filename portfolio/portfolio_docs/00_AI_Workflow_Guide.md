...
AI가 문서를 생성하거나 수정할 때는 반드시 **인간의 검증(Verification)** 단계를 거쳐야 합니다.

**Checkpoints**:
1. **분석 단계 (Scanning)**: 분석 대상 파일 목록이 정확한지 사용자 승인
2. **설계 단계 (Planning)**: 생성된 청사진(Blueprint)이나 계획이 의도와 맞는지 사용자 승인
3. **최종 산출물 (Result)**: 최종 리포트나 코드가 요구사항을 충족하는지 사용자 피드백

> **명령어**: "각 단계마다 내가 검토(Confirm)할 수 있도록 멈춰줘."
