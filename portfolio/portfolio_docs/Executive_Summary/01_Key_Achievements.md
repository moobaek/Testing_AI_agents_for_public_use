...
| 지표 | 수치 | 세부 내용 |
|:---|---:|:---|
| **독립 솔루션 개발** | 30개 이상 | AI, 플랫폼, 센서, 에너지, Healthcare, 컨설팅 등 6대 영역 |
...
### 컨설팅 프로젝트 수행 (5개)
1. **테크웰 AMS 컨설팅** - 전력량 측정계 설치, 전력 모니터링 프로그램 납품
2. **신성오토텍 AMS 컨설팅** - 사출기 데이터 수집, 사출 데이터 모니터링 프로그램 납품
3. **테크웰 데이터바우처** - 전력 데이터 분석, 데이터 라벨링
4. **대성금형 데이터바우처** - 사출 데이터 분석, 데이터 라벨링
5. **포다스 유의변수/가상변수 생성** (진행중) - 유의변수 및 가상변수 생성, 드론 고장탐구

### PM 역량 강화
- **프로젝트 관리**: 계획서 작성, 외주 관리, 완료서 작성 등 전체 라이프사이클 관리
- **컨설팅 경험**: 다양한 업체(테크웰, 신성오토텍, 대성금형, 포다스)와의 컨설팅 수행
- **데이터 분석**: 전력 데이터, 사출 데이터, 드론 데이터 등 다양한 도메인 데이터 분석 경험
