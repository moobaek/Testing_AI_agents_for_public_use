...
graph TB
    subgraph "권순룡 포트폴리오"
        ...
        subgraph "2. 전문 영역"
            AI[AI & Analytics<br/>AMS, CoCTK, FBS]
            Platform[Digital Platform<br/>DPS, 5층 아키텍처]
            IoT[Smart Sensors<br/>Edge AI, MCP]
            Energy[Energy & Safety<br/>에너지 최적화]
        end

        subgraph "3. 기술 스택"
            Python[Python<br/>49개 모듈]
            DB[Neo4j, MSSQL<br/>PostgreSQL]
            DevOps[Docker<br/>Kubernetes]
            AIAgent[Multi-Agent<br/>MCP, RAG]
        end

        subgraph "4. LLM 활용"
            Obsidian[Obsidian Design<br/>298개 문서]
            FMEA[FMEA 자동화<br/>8개 Sub-Agent]
            Prompt[프롬프트 평가<br/>25개+ 체인]
        end
    end
...
graph LR
    subgraph "5년간의 성과 (2020-2025)"
        ...
        B[논문 9편<br/>학술 검증]
        C[특허 등록<br/>기술 보호]
        D[납품 3곳<br/>세아/포미아/일본]
        E[이상탐지 93.7%<br/>데이터 품질]
        F[49개 Python 모듈<br/>엔진 개발]
        G[298개 설계 문서<br/>체계적 개발]
        H[Multi-Agent<br/>8개 협업]
    end

    A --> I[검증된<br/>데이터 엔지니어링<br/>역량]
    B --> I
    C --> I
    D --> I
    E --> I
    ...
    style I fill:#27ae60,color:#fff,stroke:#229954,stroke-width:3px
...
```mermaid
graph TB
    subgraph "AI & Analytics (6개)"
        AMS[AMS<br/>이상탐지 93.7%<br/>GS 1등급]
        CoCTK[CoCTK<br/>데이터 분석<br/>GS 1등급]
        FBS[FBS<br/>피쉬본 자동화<br/>특허 등록]
        Japan[일본 DX<br/>글로벌 기업<br/>전사 디지털화]
        Quality[품질 예측<br/>다수 업체<br/>불량률 감소]
        Defect[공정 불량<br/>예측 AI<br/>2023]
    end

    subgraph "Digital Platforms (4개)"
        DPS[DPS<br/>5층 아키텍처<br/>Neo4j 그래프DB]
        Production[생산정보 통합<br/>실시간 연계<br/>2021-2022]
        DXCenter[DX 실증센터<br/>PM 총괄<br/>2022-2023]
        Injection[사출 DX<br/>파라미터 최적화<br/>2021-2023]
    end

    subgraph "Smart Sensors & IoT (4개)"
        Sensors[스마트센서 3종<br/>보급형<br/>2022-2024]
        AISensor[AI 복합 센서<br/>Edge AI<br/>2021-2023]
        Virtual[가상센서<br/>고가센서 대체<br/>2021]
        Seah[세아특수강<br/>데이터 통합<br/>2025]
    end

    subgraph "Energy & Safety (5개)"
        Cleanroom[클린룸<br/>에너지 20% 절감<br/>PM]
        Energy[에너지 패턴<br/>메인 수행<br/>2023]
        Power[전력 예측<br/>사출 업체<br/>2021]
        DigitalTwin[EV/ESS<br/>디지털트윈<br/>2023-2024]
        PowerQuality[전력품질<br/>플랫폼<br/>2022-2023]
    end

    subgraph "Autonomous Dev Cycle (7개)"
        ObsidianOrigin[Obsidian Design<br/>298개 문서<br/>2020-2025]
        FMEAAgent[FMEA 자동화<br/>8개 Sub-Agent<br/>2024-2025]
        PromptEval[프롬프트 평가<br/>25개+ 체인<br/>2024-2025]
        FactoryOnt[Factory Ontology<br/>시각적 관리<br/>2024-2025]
        Pipeline[Pipeline System<br/>8단계<br/>2024-2025]
        TAM[TAM_Hub<br/>32개 MCP<br/>2024-2025]
        EvalFrame[Evaluation<br/>Framework<br/>2024-2025]
    end

    subgraph "핵심 기술"
        Neo4j[Neo4j<br/>그래프 DB]
        Python49[Python<br/>49개 모듈]
        Docker[Docker<br/>마이크로서비스]
        MultiAgent[Multi-Agent<br/>8개 협업]
    end
    ...
    style GS2 fill:#27ae60,color:#fff
    style MultiAgent fill:#3498db,color:#fff
```
...
graph TB
    subgraph "Frontend Layer"
        A1[React 18.3.1<br/>Web Dashboard]
    end

    subgraph "Backend Layer - Python Engine (49개 모듈)"
        B1[MLS Service<br/>Machine Learning<br/>15개 파일]
        B2[FBS Service<br/>Fishbone Analysis<br/>6개 파일]
        B3[RMS Service<br/>Range Management<br/>4개 파일]
        B4[AMS Service<br/>Integrated Analysis<br/>17개 파일]
        B5[CoCTK Service<br/>Cost Control<br/>4개 파일]
    end
...
```mermaid
graph TD
    Layer5[Layer 5: 서비스 및 UI<br/>특성요인도 시각화, 모니터링]
    Layer4[Layer 4: 통합 및 온톨로지<br/>Neo4j 그래프DB, 4M2E 관계]
    Layer3[Layer 3: AI 엔진<br/>가상 센서, 이상 검출]
    Layer2[Layer 2: 데이터 수집<br/>실시간 스트리밍, PLC/MES]
    Layer1[Layer 1: 보안 및 관리<br/>인증/권한, 로그 감사]
...
```mermaid
graph TB
    Master[Master Orchestrator<br/>Claude Code Task tool]

    subgraph "Phase 0-5 워크플로우"
        P0[Phase 0<br/>컨텍스트 수집]
        P1[Phase 1<br/>범위 정의]
        P2[Phase 2<br/>심층 분석]
        P3[Phase 3<br/>리스크 평가]
        P4[Phase 4<br/>최적화 & 문서]
        P5[Phase 5<br/>지속 개선]
    end

    subgraph "8개 독립 Sub-Agent"
        RD1[R&D Agent 1]
        RD2[R&D Agent 2]
        RD3[R&D Agent 3]
        MFG1[Mfg Agent 1]
...
```mermaid
mindmap
  root((권순룡<br/>Data Engineer<br/>5년 경력))
    Programming
      Python 5년
        49개 모듈 개발
        pandas numpy
...
```mermaid
graph TB
    subgraph "포트폴리오 문서 구조"
        Root[portfolio/portfolio_docs/]

        subgraph "핵심 문서 (00_*.md)"
            Index[00_Portfolio_Index.md]
            Profile[00_Personal_Profile.md]
            IDGuide[00_ID_System_Guide.md]
            AIGuide[00_AI_Workflow_Guide.md]
            PMGuide[00_PM_Roles_Guide.md]
        end

        subgraph "프로젝트 문서"
            Projects[02_Projects_Overview.md]
            Architecture[Architecture_Overview.md]
            Academic[04_Academic_Publications.md]
        end

        subgraph "AI 워크플로우"
            Prompts[prompts/<br/>25개+ AI 프롬프트]
            Templates[templates/<br/>11개+ 템플릿]
            Data[data/<br/>대화 컨텍스트]
        end

        subgraph "Phase 가이드"
            Phase1[Phase_1_Foundation/]
            Phase2[Phase_2_Assets/]
            Phase3[Phase_3_Expansion/]
        end
    end
...
```mermaid
graph LR
    A[사용자 질문] --> B[Portfolio_Answer_Generator_Prompt]
    B --> C[역할별 프롬프트<br/>PM/Developer/Researcher]
    C --> D[Soonryong_Answer_Generator]
    D --> E[답변 필터링]
    E --> F[최종 답변]
...
flowchart TD
    ...
    Phase8 --> Phase9[Phase 9: 온톨로지 영향 분석]
    Phase9 --> Phase10[Phase 10: 화면 설계서]
    Phase10 --> Phase11[Phase 11: 온톨로지 확장]
    Phase11 --> Phase12[Phase 12: 최종 확인]
    Phase12 --> Decision{사용자 승인}
    Decision -->|proceed| Phase13[Phase 13: 개발용 리팩토링]
    Decision -->|modify| Phase12
    Phase13 --> End([개발 준비 완료])

    style Phase0 fill:#e74c3c,color:#fff
    style Phase12 fill:#9b59b6,color:#fff
...
sequenceDiagram
    ...
    participant RD as R&D Sub-Agents (3개)
    participant Mfg as Mfg Sub-Agents (3개)
    participant QA as QA Sub-Agents (2개)
    participant Output as 최종 결과

    User->>Master: FMEA 생성 요청
    Master->>Master: Phase 0: 컨텍스트 수집
    Master->>Master: Phase 1: 범위 정의
    Master->>RD: Phase 2: R&D 심층 분석
    Master->>Mfg: Phase 2: Mfg 심층 분석
    Master->>QA: Phase 2: QA 심층 분석

    RD-->>Master: R&D 분석 결과
    Mfg-->>Master: Mfg 분석 결과
    QA-->>Master: QA 분석 결과

    Master->>Master: Phase 3: 리스크 평가 통합
    Master->>Master: Phase 4: 최적화 & 문서 생성
    Master->>Master: Phase 5: 지속 개선
    Master->>Output: FMEA 문서 완성
    Output->>User: 최종 FMEA 전달
```
...
```mermaid
graph TB
    subgraph "Testing_AI_agents_for_public_use"
        Root["📁 Repository Root"]
        
        subgraph "🤖 Claude 에이전트 시스템"
            Agent["📁 .agent/"]
            Workflows["📁 workflows/"]
            WF1["portfolio-question.md"]
            WF2["portfolio-answer-chain.md"]
            WF3["portfolio-document-modify.md"]
            WF4["portfolio-continuous-chat.md"]
        end
        
        subgraph "📚 포트폴리오 문서"
            Portfolio["📁 portfolio/portfolio_docs/"]
            Prompts["📁 prompts/ (25개+ AI 프롬프트)"]
            Assets["📁 assets/"]
            Data["📁 data/"]
            Templates["📁 templates/"]
        end
    ...
    style Workflows fill:#e74c3c,color:#fff
    style WF1 fill:#c0392b,color:#fff
    style WF2 fill:#c0392b,color:#fff
    style WF3 fill:#c0392b,color:#fff
    style WF4 fill:#c0392b,color:#fff
    style ThisDoc fill:#9b59b6,color:#fff
    style Prompts fill:#3498db,color:#fff
```
...
```mermaid
graph LR
    User["👤 사용자"] -->|"/portfolio-question"| Claude["🤖 Claude"]
    User -->|"@portfolio 언급"| Claude
    User -->|"포트폴리오 질문"| Claude
    Claude -->|"순룡 페르소나"| Answer["💬 답변"]
