...
```
Step 4: 모듈화 전략을 수행해주세요.

**Phase ID**: `phase.foundation.step04`
**목표**: ID 시스템 기반으로 모든 요소를 모듈화하고, 프롬프트와 템플릿을 독립적인 모듈로 관리

**요구사항**:
1. ID 시스템 설계 (`type.module.name` 형식)
2. 모듈 간 의존성 분석 및 관리
3. 프롬프트 모듈화 전략
4. 템플릿 표준화 가이드
...
- [[templates/Documentation_Template|문서 작성 템플릿]] (`template.documentation`)
...
**ID 규칙**:
- 모듈 ID: `module.[category].[name]`
- 프롬프트 ID: `prompt.[module].[name]`
- 템플릿 ID: `template.[module].[name]`
```

### 프롬프트 사용 예시

**사용자 요청**: "프로젝트를 모듈화하고 ID 시스템을 적용하는 방법을 알려줘"

**AI 동작**:
1. 프로젝트 구조 분석
2. ID 시스템 설계 제안
3. 모듈 간 의존성 분석
4. 프롬프트 및 템플릿 모듈화 전략 제시
5. 모듈 구조 다이어그램 생성
