...
- **해결**: 에너지 소비 패턴 분석 엔진 기반 AI 최적화 규칙 생성.
...
```
Step 1: 반복적 업무 식별을 수행해주세요.

**Phase ID**: `phase.foundation.step01`
**목표**: 매일/매주 반복하는 단순하고 규칙적인 작업을 리스트업하고, 이를 AI로 자동화할 수 있는 방법을 제시

**요구사항**:
1. 사용자의 업무 환경을 분석하여 반복적 업무 식별
2. 각 반복 업무에 대한 자동화 가능성 평가
3. AI 프롬프트 기반 자동화 방안 제시
4. 문서 중심 자동화 시스템 구축 가이드 제공

**참조 문서**:
- [[00_ID_System_Guide|ID 시스템 가이드]] (`guide.id.system`)
- [[00_AI_Workflow_Guide|자율 개발 라이프사이클]] (`guide.ai.workflow`)
- [[templates/Phase_Execution_Template|Phase 실행 템플릿]] (`template.phase.execution`)
...
**ID 규칙**:
- 반복 업무 ID: `task.repetitive.[name]`
- 프롬프트 ID: `prompt.automation.[name]`
- 문서 ID: `page.portfolio.[name]`
```

### 프롬프트 사용 예시

**사용자 요청**: "내 업무에서 반복되는 작업을 찾아서 AI로 자동화할 수 있는 방법을 알려줘"

**AI 동작**:
1. 사용자의 업무 환경 분석
2. 반복 업무 식별 및 우선순위 결정
3. 각 업무별 AI 자동화 방안 제시
4. 프롬프트 예시 제공
5. 문서 구조 제안
