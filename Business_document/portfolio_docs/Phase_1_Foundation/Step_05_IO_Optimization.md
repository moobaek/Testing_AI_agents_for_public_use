...
```
Step 5: 데이터 입출력 최적화를 수행해주세요.

**Phase ID**: `phase.foundation.step05`
**목표**: `Input File → AI Process → Output File` 흐름을 유지하여 데이터 입출력을 최적화

**요구사항**:
1. 파일 기반 I/O 아키텍처 설계
2. 적절한 파일 포맷 선택 (CSV, JSON, Markdown 등)
3. 중간 단계 파일 관리 전략
4. CLI 도구화 가이드
...
**ID 규칙**:
- 파일 ID: `file.[type].[name]`
- 프로세스 ID: `process.[module].[name]`
- CLI 도구 ID: `cli.[module].[name]`
```

### 프롬프트 사용 예시

**사용자 요청**: "AI 프로세스의 입출력을 파일 기반으로 최적화하는 방법을 알려줘"

**AI 동작**:
1. 현재 데이터 흐름 분석
2. 파일 기반 I/O 아키텍처 설계 제안
3. 적절한 파일 포맷 추천
4. 중간 파일 관리 전략 제시
5. CLI 도구 설계 가이드 제공
